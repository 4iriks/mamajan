...
from engine.slide_calc import (
    ...
    _two_row_glass_correction_adjustments,
    calculate_slide,
)
...
class TestGlassTotalCorrection:
    ...
    @pytest.mark.parametrize("panel_count", [3, 4, 5, 6])
    def test_positive_one_mm_does_not_change_central_glass(self, panel_count):
        assert _glass_correction_adjustments(panel_count, 1) == {}

    @pytest.mark.parametrize("panel_count", [3, 4, 5, 6])
    def test_positive_three_mm_changes_only_edge_glass(self, panel_count):
        assert _glass_correction_adjustments(panel_count, 3) == {
            0: 1,
            panel_count - 1: 1,
        }
    ...
    def test_positive_one_mm_does_not_raise_central_glass_or_rs2021(self):
        result = SlideCalcResult(
            panel_glass=[
        ...
        assert [panel.width_mm for panel in result.panel_glass] == [500, 480, 480]
        assert [panel.glass_profile_length for panel in result.panel_glass] == [
            497,
            477,
            477,
    ...
    def test_two_row_dimensions_use_five_mm_bubble_offsets(self):
        result = calculate_slide(
            _make_section(
                width=5000,
                height=3000,
...
class TestProfileVariables:
    ...
    def test_rpl_p_bar(self):
        """П-профиль RS1082 слева → rpl = 27.5."""
        s = _make_section(
            profile_left_p_bar=True,
        )
        r = calculate_slide(s)
        edge = _find_glass(r, "Крайние")[0]
        assert edge.width_mm == 653
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(1, 3, 0, 0, 980, 996, 996, id="one-row-zero"),
            pytest.param(1, 3, 100, 0, 952, 1052, 968, id="one-row-left"),
            pytest.param(1, 3, 0, 100, 952, 968, 1052, id="one-row-right"),
            pytest.param(1, 3, 100, 100, 924, 1024, 1024, id="one-row-both"),
            pytest.param(2, 6, 0, 0, 493, 509, 509, id="two-row-zero"),
            pytest.param(2, 6, 100, 0, 479, 579, 495, id="two-row-left"),
            pytest.param(2, 6, 0, 100, 479, 495, 579, id="two-row-right"),
            pytest.param(2, 6, 100, 100, 465, 565, 565, id="two-row-both"),
        ],
    )
...
class TestCustomerSections0107:
    ...
    def test_section_11_rs3020_does_not_create_rs122(self):
        ...
        assert _ceil_panel_widths(r) == [823, 814, 823]
        assert _ceil_panel_profile_lengths(r) == [839, 811, 839]
        assert _find_hardware(r, "RS3020")
        assert not _find_hardware(r, "RS122")
...
class TestSlideTwoRows:
    ...
    @pytest.mark.parametrize(
        ("panels", "placement", "expected_rails", "expected_numbers"),
        [
            (4, "Внешний", [1, 2, 2, 1], [2, 1, 1, 2]),
            (4, "Внутренний", [1, 0, 0, 1], [1, 2, 2, 1]),
            (6, "Внешний", [0, 1, 2, 2, 1, 0], [3, 2, 1, 1, 2, 3]),
            (6, "Внутренний", [2, 1, 0, 0, 1, 2], [1, 2, 3, 3, 2, 1]),
        ],
    )
    def test_first_two_row_panels_mirror_the_top_view(
        self, panels, placement, expected_rails, expected_numbers
    ):
        result = calculate_slide(
            _make_section(
                slide_rows=2,
        ...
        assert result.panel_rails == expected_rails
        assert result.panel_numbers == expected_numbers

    def test_center_handle_offset_keeps_full_110_mm_difference(self):
        result = calculate_slide(
            _make_section(
                width=3030,
                height=3000,
        ...
        assert result.panel_glass[1].width_mm - result.panel_glass[0].width_mm == 110
        assert not any("контроль" in warning.lower() for warning in result.warnings)

    def test_six_panel_center_offset_does_not_include_edge_recovery(self):
        result = calculate_slide(
            _make_section(
                width=4452,
                height=2516,
        ...
        assert widths == [716, 702, 802, 802, 702, 716]
        # The formula keeps the 16 mm edge recovery. The approved -3 mm
        # checksum rule then removes 2 mm from each extreme pane only.
        assert widths[0] - widths[1] == 14
        assert round(widths[2] - widths[1], 1) == 100
        assert round(widths[3] - widths[4], 1) == 100
        assert widths[-1] - widths[-2] == 14
    ...
    def test_center_rs112_uses_updated_475_center_loss(self):
        r = calculate_slide(
            _make_section(
                slide_rows=2,
                rails=3,
    ...
    def test_two_row_checksum_adjusts_only_extreme_panes(
        self, difference, expected
    ):
        assert _two_row_glass_correction_adjustments(4, difference) == expected

    @pytest.mark.parametrize(
        ("width", "expected_widths", "expected_rs2021"),
        [
            (3525, [853, 852, 852, 853], [869, 871, 868, 869]),
            (4000, [971, 971, 971, 971], [987, 990, 987, 987]),
        ],
    )
    def test_invoice_5_four_panel_rs112_control_cases(
        self, width, expected_widths, expected_rs2021
    ):
        result = calculate_slide(
            _make_section(
                width=width,
        ...
        assert [panel.width_mm for panel in result.panel_glass] == expected_widths
        assert [
            panel.glass_profile_length for panel in result.panel_glass
        ] == expected_rs2021
        assert not result.warnings

    def test_invoice_5_six_panel_case_keeps_existing_overlap_count(self):
        result = calculate_slide(
            _make_section(
                width=4679,
                height=3000,
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "Алюминиевый RS2061",
            "Прозрачный RS1006",
            "Профиль с зацепом RS3061",
        ],
    )
    def test_supported_two_row_configuration_matrix(
        self, rails, panels, inter_glass_profile
    ):
        result = calculate_slide(
            _make_section(
                width=5000,
        ...
        assert len(result.panel_glass) == panels
        assert len(result.panel_rails) == panels
        assert all(panel.width_mm == int(panel.width_mm) for panel in result.panel_glass)
        assert all(panel.height_mm == int(panel.height_mm) for panel in result.panel_glass)
        assert all(
            panel.glass_profile_length == int(panel.glass_profile_length)
            for panel in result.panel_glass
        )
        assert [panel.width_mm for panel in result.panel_glass] == [
            panel.width_mm for panel in reversed(result.panel_glass)
        ]
