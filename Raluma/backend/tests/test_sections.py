def test_create_section(client, admin_headers, project):
    r = client.post(f"/api/projects/{project['id']}/sections", headers=admin_headers, json={
        "name": "Секция A",
        "system": "СЛАЙД",
        "width": 1500,
        "height": 2200,
...
def test_section_order_increments(client, admin_headers, project):
    ...
    # Создаём 2 секции
    s1 = client.post(f"/api/projects/{project['id']}/sections", headers=admin_headers, json={
        "name": "S1", "system": "КНИЖКА",
    }).json()
    s2 = client.post(f"/api/projects/{project['id']}/sections", headers=admin_headers, json={
        "name": "S2", "system": "КНИЖКА",
    }).json()
    assert s2["order"] > s1["order"]

    # Удаляем первую
    client.delete(f"/api/projects/{project['id']}/sections/{s1['id']}", headers=admin_headers)

    # Новая секция должна иметь order > s2["order"]
    s3 = client.post(f"/api/projects/{project['id']}/sections", headers=admin_headers, json={
        "name": "S3", "system": "КНИЖКА",
    }).json()
    assert s3["order"] > s2["order"]

    # cleanup
    client.delete(f"/api/projects/{project['id']}/sections/{s2['id']}", headers=admin_headers)
    client.delete(f"/api/projects/{project['id']}/sections/{s3['id']}", headers=admin_headers)
...
def test_delete_section(client, admin_headers, project):
    s = client.post(f"/api/projects/{project['id']}/sections", headers=admin_headers, json={
        "name": "ToDelete", "system": "ЦС",
    }).json()
    r = client.delete(f"/api/projects/{project['id']}/sections/{s['id']}", headers=admin_headers)
    assert r.status_code == 204
    sections = client.get(f"/api/projects/{project['id']}/sections", headers=admin_headers).json()
