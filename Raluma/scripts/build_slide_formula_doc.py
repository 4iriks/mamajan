...
def add_notation(doc: Document) -> None:
    ...
    add_table(
        ...
        [
            ...
            ["rpl / rpr", "59,5", "59,5", "Выбран боковой профиль-замок RS2081"],
            [
                "rpl / rpr",
                "27,5",
                "27,5",
            ...
            ["pzl / pzr", "5", "5", "Выбран пузырьковый уплотнитель RS1002"],
...
def add_two_row(doc: Document) -> None:
    ...
    add_formula(
        doc,
        "CL = M + c + centr2 + cl;   CR = M + c + centr2 + cr",
        "cl = krlp и cr = krrp при заданном центральном отступе c, иначе 0. "
        "Так заданный отступ остаётся фактической разницей между крайним и "
        "центральным стеклом. Центральная левая и центральная правая панели "
        "остаются отдельными физическими позициями.",
    )
