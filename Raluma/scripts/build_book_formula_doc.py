...
def source_rows() -> list[list[str]]:
    return [
        ...
        [
            ...
            "Вычет высоты B25 с нижним компенсатором, профильные позиции, "
        ...
        [
            ...
            "Матрица высот для остальных компенсаторов и систем "
            "B16/B17/C16/C17; совместимость старых полей",
...
def variable_rows() -> list[list[str]]:
    return [
        ...
        ["S", "Профильная система", "—", "B25 / B16 / B17 / C16 / C17"],
...
def geometry_rows() -> list[list[str]]:
    return [
        ...
        [
            "Высота стекла",
            "Hст = H − Δ(S, C)",
            "Прямая секция; вычет зависит от системы и компенсатора",
            "Матрица ниже: Excel → legacy",
            "excel/legacy\nПРЕДВАРИТЕЛЬНО",
        ],
        [
            "Высота дверного стекла",
            "B25: Hд = Hст; B16-семейство: Hд = Hст − 33",
            "Физические панели с ролью двери",
            "Excel B25; legacy B16-семейство",
            "excel/legacy\nПРЕДВАРИТЕЛЬНО",
        ],
        [
            "Высота панели при склейке",
            "B25: Hсб = Hст + 33; B16-семейство: Hсб = Hст − 33",
            "Все физические панели выбранной системы",
            "Excel B25; legacy B16-семейство",
            "excel/legacy\nПРЕДВАРИТЕЛЬНО",
        ],
        [
...
def height_matrix_rows() -> list[list[str]]:
    return [
        [
            "B25",
            "135",
...
def opening_rows() -> list[list[str]]:
    return [
        ...
        ["4", "Высотные вычеты", "Подтвердить матрицу Δ(S, C): B25 с верхним/обоими/без компенсатора и перенос семейства B16 на B17/C16/C17.", "Влияет на стекло"],
...
def height_example_rows() -> list[list[str]]:
    rows = []
    examples = [
        ("B25", "нижний", 135.0, 0.0, 33.0, "excel"),
        ("B25", "оба", 115.0, 0.0, 33.0, "legacy"),
        ("B16", "нижний", 102.0, -33.0, -33.0, "legacy"),
        ("B17", "верхний", 130.0, -33.0, -33.0, "legacy"),
        ("C16", "без", 117.0, -33.0, -33.0, "legacy"),
        ("C17", "оба", 115.0, -33.0, -33.0, "legacy"),
    ]
    height = 2500.0
    for system, compensator, deduction, door_delta, assembly_delta, source in examples:
        glass = height - deduction
        rows.append(
            [
                system,
                compensator,
                "2500,0",
                f"{glass:.1f}".replace(".", ","),
                f"{glass + door_delta:.1f}".replace(".", ","),
                f"{glass + assembly_delta:.1f}".replace(".", ","),
                f"{source} · ПРЕДВАРИТЕЛЬНО",
            ]
        )
    return rows
...
def build_document() -> Document:
    ...
    add_paragraph(
        doc,
        "Матрица Δ(S, C), мм. Система выбирается и хранится явно; B17/C16/C17 "
        "временно используют восстановленное семейство высот B16.",
        bold_prefix="Матрица Δ(S, C), мм.",
    )
    add_table(
        doc,
        [
        ...
        height_matrix_rows(),
        [900, 800, 800, 800, 800, 1450, 1450, 2360],
        status_col=7,
        mono_cols={0, 1, 2, 3, 4, 5, 6},
        center_cols={0, 1, 2, 3, 4, 5, 6},
        font_size=7.4,
    )
    add_callout(
        doc,
        "Граница подтверждения систем",
        "B25 с нижним компенсатором восстановлен из нового Excel. Остальные "
        "варианты B25 и все B16/B17/C16/C17 взяты из старой программы, имеют "
        "статус preliminary и блокируют производственные документы КНИЖКИ.",
        LIGHT_YELLOW,
    )
    add_callout(
        doc,
    ...
    add_table(
        ...
        [
            ["Профильная система", "B25 / B16 / B17 / C16 / C17", "Явное поле секции; старые «С/Без каретки» переносятся в B25 с предупреждением", "B25 — ПОДТВЕРЖДЕНО; остальные — ПРЕДВАРИТЕЛЬНО"],
    ...
    add_table(
        ...
        [
            ["Прямая B25, 2…6 панелей", "confirmed", "Ширина — confirmed; высота — preliminary", "КНИЖКА: следующий пакет"],
            ["Прямая B16/B17/C16/C17", "preliminary", "Доступна по legacy-матрице высот", "ЗАБЛОКИРОВАНО для КНИЖКИ"],
            ...
            ["Смешанный проект", "по секциям", "СЛАЙД/ЛИФТ продолжают рассчитываться", "Стекло/покраска исключают КНИЖКУ с предупреждением"],
        ],
        [2800, 1900, 2600, 2060],
        status_col=3,
        font_size=8.3,
    ...
    add_paragraph(
        doc,
        "Контроль высоты: H = 2500,0 мм. Значения ниже проверяют явную систему, "
        "вариант компенсатора, высоту стекла, дверного стекла и панели при склейке.",
        bold_prefix="Контроль высоты:",
    )
    add_table(
        doc,
        ["S", "Компенсатор", "H, мм", "Hст, мм", "Hд, мм", "Hсб, мм", "Источник / статус"],
        height_example_rows(),
        [850, 1350, 1100, 1100, 1100, 1100, 2760],
        status_col=6,
        mono_cols={0, 2, 3, 4, 5},
    ...
    add_table(
        ...
        [
            [
                ...
                "исключение КНИЖКИ из поддержанных частей смешанных документов.",
