...
OUTPUT = Path(
    os.environ.get(
        ...
        ROOT / "docs" / "ЛИФТ_формулы_зависимости_и_несостыковки.docx",
    )
)
...
def add_title_page(doc: Document) -> None:
    ...
    run = subtitle.add_run("Формулы, зависимости и несостыковки Excel")
    set_run_font(run, size=15, color=MUTED, bold=True)

    metadata = [
        ("Проект", "RALUMA"),
        ("Назначение", "Согласование расчётной логики системы ЛИФТ"),
        ("Версия", "1.0 - сводка по исходным Excel"),
        ("Дата", date.today().strftime("%d.%m.%Y")),
        ("Статус", "Формулы систематизированы. Противоречия вынесены отдельно."),
    ]
    ...
    run = intro.add_run(
        "Документ содержит только расчётные формулы системы ЛИФТ, зависимости "
        "входных параметров, артикулов и результатов, а также отдельный перечень "
        "несостыковок исходных Excel. Отчёты о реализации, тестировании и выпуске "
        "в документ не включаются."
    )
    ...
    add_status_box(
        doc,
        "ФОРМУЛЫ",
        "Расчётные выражения приведены в единой системе обозначений W, H, P, Q, F и M.",
        LIGHT_GREEN,
    )
    add_status_box(
        doc,
        "ЗАВИСИМОСТИ",
        "Для каждой группы указано, какие параметры, варианты заполнения и управления влияют на результат.",
        LIGHT_BLUE,
    )
    add_status_box(
        doc,
        "НЕСОСТЫКОВКИ",
        "Конфликты и отсутствующие правила исходных Excel собраны в отдельном разделе без догадок.",
        LIGHT_YELLOW,
    )

    add_heading(doc, "Состав документа", 2)
    add_bullet(doc, "Формулы геометрии, заполнений, профилей, фурнитуры, цепи и привода.")
    add_bullet(doc, "Зависимости от количества панелей, глухих створок, заполнения и управления.")
    add_bullet(doc, "Подтверждённые исключения для отдельных артикулов и применений.")
    add_bullet(doc, "Противоречия Excel, которые требуют решения заказчика.")
...
def add_notation_and_architecture(doc: Document) -> None:
    add_heading(doc, "1. Переменные и зависимости", 1)
    add_table(
        doc,
    ...
    add_heading(doc, "Ключевые зависимости", 2)
    add_table(
        doc,
        ["Входной параметр", "Что определяет"],
        [
            [
                "W, H",
                "Размеры рамы, заполнений, панелей при склейке, профилей, цепи и площадь.",
            ],
            [
                "P",
                "Вариант формул для 2, 3 или 4 панелей и состав профильной системы.",
            ],
            [
                "F и M = P - F",
                "Количество подвижных панелей, расчёт веса и крутящего момента.",
            ],
            [
                "Тип заполнения",
                "Формулы геометрии, профильная линейка, эквивалентная толщина и масса.",
            ],
            [
                "Пульт ДУ / кнопка",
                "Тип и количество приводов, пультов, кнопок и элементов синхронизации.",
            ],
            [
                "Сторона ввода кабеля",
                "Комплект левых/правых боковых крышек и положение привода.",
            ],
            [
                "Q",
                "Итоговые количества профилей, фурнитуры, крепежа и секций.",
            ],
        ],
        [3000, 7062],
        font_size=9,
...
def add_common_profiles(doc: Document) -> None:
    add_heading(doc, "2. Общие профили рамы", 1)
    add_status_box(
        doc,
        "ПОДТВЕРЖДЕНО",
    ...
    add_bullet(
        doc,
        "RL104 окрашивается при любом применении и при любой расчётной длине, включая W - 155 и W - 62.",
    )
...
def add_panel_formulas(doc: Document) -> None:
    add_heading(doc, "3. Заполнение и панели при склейке", 1)
    paragraph = doc.add_paragraph()
    run = paragraph.add_run(
    ...
    doc.add_page_break()
    add_heading(doc, "3.1. Две панели - стекло 8 мм", 2)
    ...
    add_heading(doc, "3.2. Две панели - стеклопакет 20 мм", 2)
    add_bullet(
        doc,
    ...
    add_heading(doc, "3.3. Три панели - стекло 8 мм", 2)
    ...
    add_heading(doc, "3.4. Три панели - стеклопакет 20 мм", 2)
    ...
    add_heading(doc, "3.5. Четыре панели - обычное открывание", 2)
    add_status_box(
        doc,
        "ЧАСТИЧНО ПОДТВЕРЖДЕНО",
        "В конечном размере заполнения необходимо вычитать 1 мм. Точная исходная формула для четырёх панелей ещё уточняется заказчиком.",
        LIGHT_YELLOW,
    )
    add_formula(
        doc,
    ...
    add_heading(doc, "3.6. Четыре панели - верх и низ глухие", 2)
    ...
    add_heading(doc, "3.7. Профили панелей", 2)
    add_table(
        doc,
        ...
        [
            ...
            [
                "Стеклопакет 20 мм, любое число панелей",
                "RL123, RL122, RL1241, RL1211, RL105",
                "Согласованное правило: профильная линейка не меняется в зависимости от числа панелей.",
            ],
            [
                "4 панели",
                "RL105",
...
def add_hardware(doc: Document) -> None:
    add_heading(doc, "4. Фурнитура ЛИФТ", 1)
    add_status_box(
        doc,
        "ПОДТВЕРЖДЕНО",
        "Формулы фурнитуры берутся из колонки Excel с текстовым описанием. Формулы щёточных уплотнителей RU004 и RU006 дополнительно уточнены заказчиком.",
        LIGHT_GREEN,
    )
    rows = [
        ...
        ["RU1039", "Наклейка", "Q"],
        ["RL150", "Инструкция ЛИФТ", "1 шт. на проект"],
    ]
    add_table(
        doc,
    ...
    add_table(
        ...
        [
            [
                "RU004, щётка 7x6",
                "(Wmax x P x 2 x Q) / 1000 м",
                "Wmax — максимальная ширина физической панели",
            ],
            [
                "RU006, щётка 7x12",
                "(L_RL102 x 12 x Q) / 1000 м",
                "L_RL102 = H - 161 мм",
            ],
            ...
            [
                "DIN7504O 3,9x13",
                "CEILING(H / 300) x 3 x Q + 5Q",
                "Округление H / 300 вверх до целой единицы",
            ],
        ],
        [2700, 4700, 2662],
        font_size=8.7,
...
def add_torque(doc: Document) -> None:
    add_heading(doc, "5. Крутящий момент и привод", 1)
    add_formula(
        doc,
        "M = P - F",
    ...
    add_formula(
        ...
        "Вес для вывода округляется вверх до 0,1 кг, но момент считается от исходного неокруглённого веса.",
    )
    add_formula(
        doc,
        "T = Вес x 9,81 x 51 / 1000",
        "Результат округляется вверх и выводится с точностью 0,1 Н·м.",
    )
    add_table(
        doc,
    ...
    add_status_box(
        doc,
        "ПОДТВЕРЖДЕНО",
        "При T <= 80 Н·м используется один привод; при T > 80 Н·м — два привода; при T > 160 Н·м дополнительно выводится предупреждение.",
        LIGHT_GREEN,
    )
...
def add_documents_plan(doc: Document) -> None:
    ...
    add_numbered(
        doc,
        "Страница 2: нарезка профилей, фурнитура, крепёж, привод, вес, крутящий момент, предупреждения и комментарии.",
    )
    add_numbered(
        doc,
    ...
    add_bullet(
        doc,
        "В двухпанельной конструкции заказывать стеклопакет только для подвижной панели; глухая панель заполняется Пеноплексом 20 мм и в заявку на стекло не попадает.",
    )
    ...
    add_bullet(
        doc,
        "В двухпанельной конструкции со стеклопакетом Пеноплекс 20 мм выводить отдельной группой от стеклопакета.",
    )
...
def add_qa_plan(doc: Document) -> None:
    ...
    steps = [
        ...
        "Создан отдельный двухстраничный производственный лист ЛИФТ с тремя схемами.",
    ...
    add_status_box(
        doc,
        "РЕЗУЛЬТАТ ПРОВЕРОК",
        "Backend: 374 passed, 2 skipped. Ruff, TypeScript typecheck, frontend build, "
...
def add_open_questions(doc: Document) -> None:
    add_heading(doc, "6. Подтверждённые зависимости и исключения", 1)
    confirmed = [
        (
            "C1",
            "Стеклопакет: вес и момент",
            "Эквивалентная толщина 12 мм прямо используется в формулах файла «фурнитура ЛИФТ.xlsx».",
        ),
        (
            "C2",
            "Профильные серии заполнений",
            "Для любого стекла 8 мм используются RL113/RL112/RL115/RL114. Для любого стеклопакета 20 мм используются RL123/RL122/RL1241/RL1211. Число панелей правило не меняет.",
        ),
        (
            "C3",
            "Окраска RL104",
            "RL104 окрашивается независимо от применения и расчётной длины.",
        ),
        (
            "C4",
            "Финальный вычет",
            "В конечном размере заполнения вычитается 1 мм. Полная формула четырёхпанельного варианта ещё уточняется.",
        ),
        (
            "C5",
            "DIN7504O 3,9x13",
            "Промежуточное значение H / 300 округляется вверх до целой единицы.",
        ),
        (
            "C6",
            "Инструкция RL150",
            "Одна инструкция на весь проект, независимо от количества секций.",
        ),
        (
            "C7",
            "Граница привода",
            "T <= 80 Н·м — один привод; T > 80 Н·м — два привода; T > 160 Н·м — предупреждение.",
        ),
        (
            "C8",
            "Щёточные уплотнители",
            "RU004 7x6 = (максимальная ширина панели x P x 2 x Q) / 1000 м. RU006 7x12 = (длина RL102 x 12 x Q) / 1000 м, где длина RL102 = H - 161 мм.",
        ),
    ]
    add_table(
        doc,
        ["ID", "Тема", "Зафиксированное правило"],
        [list(row) for row in confirmed],
        [800, 2900, 6362],
        font_size=9,
    )

    add_heading(doc, "7. Противоречия и отсутствующие правила Excel", 1)
    questions = [
        (
            "Q1",
            "Четыре панели: размеры заполнений",
            "Подтверждено, что в конечном результате вычитается 1 мм. Требуется окончательная исходная формула ширины и высоты: Excel содержит несовместимые варианты W - 134 / W - 135 и разные варианты вычета по высоте.",
        ),
        (
            "Q2",
            "RL105 для четырёх панелей",
            "Во всех исходных Excel указаны только готовые длины 1903 и 2823 для примера H = 3950 мм. Нужна общая формула либо второй эталон с другой высотой.",
        ),
    ]
    add_table(
        doc,
        ["ID", "Несостыковка", "Что требуется определить"],
    ...
    add_status_box(
        doc,
        "ВАЖНО",
        "Правила C1-C8 подтверждены ответами заказчика и зафиксированы как приоритетные. "
        "Для Q1-Q2 исходные файлы не дают единственного ответа: решение необходимо "
        "зафиксировать отдельно до окончательного согласования расчётов.",
        LIGHT_RED,
    )
...
def add_change_log(doc: Document) -> None:
    ...
    add_table(
        ...
        [
            ...
            [
                ...
                "Реализованы отдельный расчёт ЛИФТ, каталог, производственный лист, заявка на стекло, покраску, накладная и тесты.",
                "Реализовано",
            ],
            [
                "1.1",
                date.today().strftime("%d.%m.%Y"),
                "Уточнены двухпанельный стеклопакет с Пеноплексом, округление момента, контроль геометрии и нумерация стекол; ПЛ приведён к двум страницам.",
                "Реализовано",
            ],
            [
                "1.2",
                date.today().strftime("%d.%m.%Y"),
                "Все исходные Excel перепроверены повторно: подтверждены профильная линейка четырёхпанельного стеклопакета, эквивалентная толщина 12 мм и зависимость окраски RL104 от применения; противоречия источника вынесены отдельно.",
                "Проверено по Excel",
            ],
        ],
        [1100, 1600, 5762, 1600],
    ...
    paragraph = doc.add_paragraph()
    run = paragraph.add_run(
        "Следующая версия зафиксирует ответы заказчика на Q1-Q7 и результат "
...
def build() -> Path:
    ...
    doc.core_properties.title = "ЛИФТ - формулы, зависимости и несостыковки"
    doc.core_properties.subject = "RALUMA: согласование формул ЛИФТ по Excel"
    doc.core_properties.author = "RALUMA"
    doc.core_properties.keywords = "ЛИФТ, формулы, зависимости, противоречия, RALUMA"
    doc.save(OUTPUT)
    return OUTPUT
